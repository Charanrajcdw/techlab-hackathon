...
def generatePDf(imagebuffersdict, summary, keypoints):
    ...
    sub_title = Paragraph("XL Summary", sub_title_black)
    ...
    # PAGE - 2 :: Key points
    sub_title = Paragraph("Few Key Points", sub_title_black)
    elements.append(sub_title)
    ...
    for index, imagebufferobj in enumerate(imagebuffersdict):
        ...
        print(f'imagetitle :: {imagebufferobj.get("title")}')
        print(f'imagedict :: {imagebufferobj} ')
        if index % 2 == 0:
            # Add title to the elements list

            title = Paragraph(imagebufferobj.get('title'), sub_title_black)
            elements.append(title)
